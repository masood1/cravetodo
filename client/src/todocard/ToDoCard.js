    ...
    render() {
        return (
            <div className="todocard" >
                ...
                <Modal show={this.state.show} handleClose={this.handleClose} title="Edit To-Do">
                    <EditCheckListContainer stateData={this.props} callback={this.handleClose}/>
                </Modal>
            </div>
        );
    }
...
class EditCheckListContainer extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = { name: "", todos: [{ id: 1, todo: "" }], counter: 1, error: null, message: null, todoid: null };
    ...
    componentDidMount(){
        ...
        this.setState({name:this.props.stateData.name,todos:todoEntries,counter:counter,todoid:this.props.stateData.todoid})
    }

    addNewToDo() {
        this.state.todos.push({ id: this.state.counter, todo: "",completed:false})
    ...
    updateCompleted(id) {
        ...
        console.log(id)
        this.state.todos[arrayToObject.indexOf(id)]["completed"] = !this.state.todos[arrayToObject.indexOf(id)]["completed"] 
        this.setState({ todos: this.state.todos })
    }
                ...
                <div style={{marginTop:"4%"}}>
                    {this.state.todos.map(entry => (
                        <div className="editCheckBox"  key={entry.id} >
                            <label><input type="checkbox" value={entry.completed} checked={entry.completed}  onChange={(event) => this.updateCompleted(entry.id)}/>
